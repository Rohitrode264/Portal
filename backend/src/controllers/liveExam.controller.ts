...
export class LiveExamController {
  ...
  async markAttendance(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (allowedStudentCpIds !== null && !allowedStudentCpIds.includes(studentCpId)) {
          res.status(403).json({ error: "Forbidden: You are not authorized to mark attendance for this student." });
          return;
      }

      const isLive = examObj.status === "LIVE";

      const session = await ExamSession.findOne({ examId, studentCpId });
      
      // Check if student is actively waiting (heartbeat within last 60 seconds)
      const isWaiting = session && 
                        session.status === "ABSENT" && 
                        session.heartbeatLastSeen && 
                        (Date.now() - new Date(session.heartbeatLastSeen).getTime() < 60000);

      if (!isWaiting) {
        res.status(400).json({ error: "Student is not currently waiting online to join." });
        return;
      }

      session.status = isLive ? "IN_PROGRESS" : "PRESENT";
      session.markedPresentAt = new Date();
      if (isLive) {
      ...
      res.json({
        message: isLive
          ? "Late student admitted and exam started immediately!"
  ...
  async releaseAllPresent(req: Request, res: Response): Promise<void> {
    try {
      ...
      const filter: any = { examId: id, status: "PRESENT" };
      if (allowedStudentCpIds !== null) {
          filter.studentCpId = { $in: allowedStudentCpIds };
      }
      ...
      const presentSessions = await ExamSession.find({
        examId: id,
        status: "IN_PROGRESS",
      });
      ...
      await Exam.findByIdAndUpdate(id, {
        status: "LIVE",
        scheduledAt: liveStart,
      });
      res.json({
        message:
          "Attendance submitted. Exam launched for all present students.",
      ...
      res
        .status(500)
        .json({ error: "Failed to release exam for present students" });
    }
  }
  ...
  async getLiveStatus(req: Request, res: Response): Promise<void> {
    try {
      ...
      const data = await sectionService.getSectionsData(exam.classId, exam.group as 'PCM' | 'PCB');
      
      const allowedStudents: any[] = [];
      let isCoordinator = false;

      for (const section of data.sections) {
         if (userRole === "ADMIN" || section.coordinatorCpId === userCpId) {
             allowedStudents.push(...section.students);
             if (section.coordinatorCpId === userCpId) isCoordinator = true;
         }
      }
      ...
      const roster = allowedStudents.map((student) => {
        const session = sessions.find(
          (s) => s.studentCpId === student.admissionNumber,
        );
        return {
  ...
  async startExam(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!exam || (exam.status !== "LIVE" && exam.status !== "PUBLISHED")) {
        res
          .status(400)
          .json({ error: "Exam is not currently published or live" });
        return;
      }
      ...
      if (session.status === "PRESENT") {
        ...
        res.status(202).json({
          waitingApproval: true,
          message:
            "Verified present! Waiting for coordinator to launch the exam paper.",
        });
        return;
        ...
        if (!session.lockedSessionId) {
          ...
          res.status(403).json({
            error:
              "SECURITY LOCKOUT: Exam started on another device/session. You cannot resume here.",
          });
          return;
        }
      ...
      if (exam.status === "LIVE") {
        const startRef = exam.scheduledAt
  ...
  async heartbeat(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (
        !session ||
        (session.status !== "IN_PROGRESS" && session.status !== "ABSENT")
      ) {
...
export const autoEndExams = async () => {
  try {
    const liveExams = await Exam.find({ status: "LIVE" });
    const now = Date.now();
    ...
    for (const exam of liveExams) {
      const startRef = exam.scheduledAt
        ? new Date(exam.scheduledAt).getTime()
        : new Date(exam.updatedAt).getTime();
      const examEndTime = startRef + exam.duration * 60 * 1000;

      // Add 10 seconds buffer to let frontend submissions arrive
      if (now >= examEndTime + 10000) {
        console.log(
          `Auto-ending exam ${exam._id} (${exam.title}) as time is up.`,
        );
        ...
        // Release all exam locks for students of this exam
        const allSessions = await ExamSession.find({ examId: exam._id });
        const studentCpIds = allSessions.map((s) => s.studentCpId);
        if (studentCpIds.length > 0) {
          await Session.updateMany(
            { userId: { $in: studentCpIds } },
    ...
    console.error("autoEndExams cron error:", error);
  }
};
